...
def main() -> None:
    ...
    for index, task in enumerate(selected_tasks, start=1):
        ...
        if result.get("key_metrics"):
            safe_print(f"  key_metrics: {result['key_metrics']}")
    ...
    safe_print(f"  Total tasks:                  {len(rows)}")
    safe_print(
        f"  Tasks with available reports: "
        f"{sum(1 for row in rows if is_report_available_result(row))}"
    )
    safe_print(f"  Failed or missing tasks:      {failed_or_missing_tasks}")
